---
name: cert-exam
description: Interactive Claude Certified Architect Foundations practice exam. Presents questions one by one using AskUserQuestion, tracks answers, and gives a final scaled score (100-1000, pass at 720). Run from a repo containing "Claude Certification/Claude Certification Exam.md".
...
The invocation argument is: `$ARGUMENTS`. If it contains a number (e.g., `/cert-exam 20`), treat that number as the fixed question count and **skip Question 1 below** — do not ask the count again. If `$ARGUMENTS` is empty, ask Question 1 and default the recommendation to 20.

## Step 2: Configure the session

Use ONE AskUserQuestion call with the questions below (omit Question 1 if the count was already set by the argument, leaving up to 2 questions):

**Question 1** — How many questions? *(skip if a count argument was provided)*
- header: "Questions"
